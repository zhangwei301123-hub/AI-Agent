...
- 反潜飞机距潜艇超过 0.4 海里时只追踪接近，进入 0.4 海里（含边界）后才允许发射鱼雷；
- 飞机和潜艇需要严格小于 30°，水面舰艇免除朝向限制；
- 50 km 内的合法导弹目标优先于其他目标，否则选择攻击范围内最近合法目标；
- 同一目标最多 3 个攻击者；按“武器 GUID—发射平台—目标”分别跟踪，在某个平台最后一发在途武器消失时只释放该平台槽位，其他平台仍在途不会阻止补位；发射后 10 帧未形成武器实体时释放，600 帧为最终兜底；
- 同一导弹目标生命周期内累计最多 4 发拦截弹；
- 健康可控且处于停放状态的飞机主动起飞，并防止逐帧重复下发；
- 在空飞机燃油不高于 20%，或已安装的打击武器耗尽时返航；
- 已有攻击的目标/授权/安全/后勤条件消失时主动调用 `cancelAttackw`；
- 默认通过 `getMissionList` 获取唯一巡逻区域，并以包含边界的多边形判断飞机是否在区内；
- 在区内、距海面 0～500 m 且浮标库存大于0时首次部署浅层主动声呐浮标；后续按 `14.816 km` 航迹累计间隔部署，不检查附近已有浮标；巡逻路径由系统任务管理，符号推理不再下发航路。
...
- `state.py`：维护并发攻击槽位、起飞/返航防重复窗口、按发射平台跟踪在途武器并逐个释放槽位，以及累计拦截弹数量；
...
匹配任务，保留 `missionName`、`missionType` 和 `areaPoints`。当前只有一个有效巡逻
任务时，如果反潜巡逻机的态势没有返回 `missionId`，算法会自动绑定这个唯一任务。
之后只使用任务多边形判断飞机当前位置是否位于区域内，边界也算区域内。

```text
getMissionList 唯一巡逻区域 → 点在多边形内（含边界）
    → 距海面高度 0～500 m → 浮标库存大于0
    → 首次部署或距上次成功部署后的累计航程 >= 14.816 km
    → delpoySonobuoyw(passiveOrActive=True, shallowOrDeep=True)
```

`passiveOrActive=True` 明确选择主动声呐浮标，`shallowOrDeep=True` 明确选择浅层
（接口说明为最大水下约 40 m）。当前规则不读取附近已有浮标，也不依据其覆盖范围
或有效期阻断部署。首次成功部署后，按相邻态势点累计飞机实际航迹；达到 `14.816 km`
（原 `7.408 km` 的 2 倍）才允许再次部署，成功后清零累计航程。由于累计的是航迹而非
到最近浮标的直线距离，飞机绕航回到旧位置时仍会按间隔重新部署。

巡逻路径、速度和高度由系统现有巡逻任务自行管理。符号推理不会生成多点坐标，
也不会为巡逻任务调用 `setUnitRoutew`。
