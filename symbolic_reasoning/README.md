...
- 在区内、距海面 0～500 m 且超出已有主动浮标覆盖时，只部署浅层主动声呐浮标；巡逻路径由系统任务管理，符号推理不再下发航路。
...
getMissionList 唯一巡逻区域 → 点在多边形内（含边界）
    → 距海面高度 0～500 m → 不在已有主动浮标覆盖范围内
    → delpoySonobuoyw(passiveOrActive=True, shallowOrDeep=True)
```

`passiveOrActive=True` 明确选择主动声呐浮标，`shallowOrDeep=True` 明确选择浅层
（接口说明为最大水下约 40 m）。算法读取态势中主动浮标的
`rangeSensor_UnderWater[].Radius`，按海面水平距离判断覆盖；缺少半径时使用
`7.408 km` 默认值。被动浮标不计入主动覆盖。部署 RPC 成功后会立即预留覆盖点，
避免新浮标实体尚未出现在下一帧态势时重复投放。
